...
## Needed because `shamsi:` holds Jalali (Persian) dates like 1405-02-31, which
## are valid Jalali but invalid Gregorian, and PyYAML's default timestamp
## resolver raises "day is out of range for month" trying to parse them.
...
class NoDatesYAMLHandler(frontmatter.default_handlers.YAMLHandler):
    def load(self, fm, **kwargs):
        kwargs["Loader"] = NoDatesSafeLoader
...
def find_publish_mds(md, ignore):
    ...
    # load md file
    try:
        post = frontmatter.load(md, handler=NoDatesYAMLHandler())
