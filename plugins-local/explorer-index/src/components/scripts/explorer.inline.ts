...
// ---------------------------------------------------------------------------
// Link-based tree builder.
//
// Instead of building a tree from the folder/slug structure, we build it from
// the link graph:
//   - top-level items  = notes whose slug starts with `indexPrefix` (your MOCs)
//   - children of each  = the notes that link TO that MOC (its backlinks)
//
// Nodes are plain objects exposing exactly the fields `renderTree` reads:
//   { isFolder, slug, slugSegment, displayName, data, children }
// ---------------------------------------------------------------------------

// Title displayed for a content entry, falling back to the last slug segment.
function entryDisplayName(slug, entry) {
  const title = entry && entry.title && entry.title !== "index" ? entry.title : null;
  return title || slug.split("/").pop() || slug;
}

// A clickable leaf (a backlink note).
function makeLeafNode(slug, entry) {
  return {
    isFolder: false,
    slug,
    slugSegment: slug.split("/").pop() || slug,
    displayName: entryDisplayName(slug, entry),
    data: { ...entry, slug },
    children: [],
  };
}

// A top-level MOC. Rendered as an expandable folder when it has backlinks,
// otherwise as a plain link. Either way the title links to the MOC's own page.
function makeIndexNode(slug, entry, children) {
  return {
...
async function buildFileTrie(dataFns, indexPrefix) {
  try {
    ...
    const prefix = indexPrefix || "10-19-inputs/19-index/";

    // Invert the link graph: backlinks[target] = [source slugs that link to it].
    const backlinks = new Map();
    for (const [srcSlug, entry] of entries) {
      const links = (entry && entry.links) || [];
      for (const target of links) {
        if (target === srcSlug) continue; // ignore self-links
        if (!backlinks.has(target)) backlinks.set(target, new Set());
        backlinks.get(target).add(srcSlug);
      }
    }
    ...
    // Top-level = notes under the index prefix (excluding the folder's own index page).
    const topLevel = [];
    for (const [slug, entry] of entries) {
      if (!slug.startsWith(prefix)) continue;
      if (slug === prefix + "index") continue;

      const sources = [...(backlinks.get(slug) || [])];
      const children = sources
        .map((srcSlug) => makeLeafNode(srcSlug, contentData[srcSlug]))
        .sort(byTitle);

      topLevel.push(makeIndexNode(slug, entry, children));
    }

    topLevel.sort(byTitle);
    console.log("[Explorer] Index (top-level) items:", topLevel.length);

    // `dataFns` (sort/filter/map) are intentionally ignored here — the tree
    // topology is defined by links, not by the folder-based defaults.
    void dataFns;
    void defaultSortFn;
    ...
    return { isFolder: true, slug: "", slugSegment: "", children: topLevel };
  } catch (e) {
